...
while True:
    ...
    for d in range(0,2 * cam.width * cam.height,4):
        if ((d>(cam.width*2*7-1) and d<(cam.width*2*8)) or (d>(cam.width*2*14-1) and d<(cam.width*2*15)) or (d>(cam.width*2*21-1) and d<(cam.width*2*22))):
            u = buf[d+1] - 128
            v = buf[d+3] - 128
    ...
    for c in range(red.size):
        if ((c>(cam.width*7-1) and c<(cam.width*8)) or (c>(cam.width*14-1) and c<(cam.width*15)) or (c>(cam.width*21-1) and c<(cam.width*22))):
